...
try {
  ...
  searchPrincipal.addEventListener('input', (e) => {
    ...
    if (e.target.value.length >= 3 || e.inputType === "deleteContentBackward") { //"deleteContentBackward" supprime le contenu juste avant la position du signe d'insertion
      search(Tags.active, Recipe.instances);
    }
  })
} catch (error) { // intanciation du bloc de code à exécuter si une erreur se produit dans le bloc try
